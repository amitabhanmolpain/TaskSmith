...
"""
    ...
    try:
        ...
        return fallback if fallback else ["1. Define implementation task | File: backend/services/ai_service.py | Description: Refine prompt and parsing logic | Goal: Return structured micro-tasks reliably."]

    except Exception:
        return [
            "1. Check the input task text | File: backend/controllers/task_controller.py | Description: Make sure the request includes the main task text before sending it to the AI service. | Goal: Prevent empty or unclear requests.",
            "2. Update the AI prompt | File: backend/services/ai_service.py | Description: Tell the AI to write tasks in simple words and keep the File, Description, and Goal format. | Goal: Make the output easy to read.",
            "3. Check the API response | File: backend/controllers/task_controller.py | Description: Confirm the backend returns the micro_tasks list in the expected format. | Goal: Keep the frontend and backend working together.",
            "4. Review task display in the UI | File: src/components/MicroTaskList.tsx | Description: Make sure the generated task text is easy to scan in the frontend list. | Goal: Help developers read tasks faster.",
            "5. Run a quick test | File: backend/test.py | Description: Send one sample task to the AI service and review the returned micro-tasks. | Goal: Catch prompt or parsing problems early."
        ]